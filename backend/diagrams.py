...
def _beam_response(el: dict, x: float) -> tuple[float, float]:
    x0,x1=float(el["x0"]),float(el["x1"]); L=x1-x0
    if L <= 0:return float(el["v0_mm"]),float(el["theta0_rad"])
    z=min(1.0,max(0.0,(x-x0)/L)); v0=float(el["v0_mm"])/1000;v1=float(el["v1_mm"])/1000;t0=float(el["theta0_rad"]);t1=float(el["theta1_rad"])
    n1=1-3*z*z+2*z**3;n2=z-2*z*z+z**3;n3=3*z*z-2*z**3;n4=-z*z+z**3
...
def build_diagrams(model: dict, result: dict, samples_per_segment: int = 16) -> dict:
    supports=result.get("reactions",[]);loads=model.get("loads",[]);total=sum(float(s["length"]) for s in model.get("spans",[]))
    boundaries={0.0,total}
    for s in supports:boundaries.add(float(s["position"]))
    for load in loads:
        boundaries.add(float(load["position"]))
        if _kind(load.get("type")) in {"udl","distributed","uniform"}:boundaries.add(float(load["to"]))
    ...
    def shear_moment(x):
        ...
        for r in supports:
            p=float(r["position"])
            if p<=x+1e-10:
                R=float(r.get("vertical_kN",0.0));V+=R;M+=R*(x-p)
        for load in loads:
            t=_kind(load.get("type"));p=float(load.get("position",0.0))
            if t in {"point","point_load"} and p<=x+1e-10:
                P=-_point_vertical(load);V+=P;M+=P*(x-p)
            elif t=="moment" and p<=x+1e-10:M+=float(load.get("value",0.0))
            elif t in {"udl","distributed","uniform"}:
                dv,dm=_udl_result(load,x);V+=dv;M+=dm
        return V,M
    ...
    for a,b in zip(xs,xs[1:]):
        for j in range(samples_per_segment):
            x=a+(b-a)*j/samples_per_segment;V,M=shear_moment(x)
    ...
    if xs:
        x=xs[-1];V,M=shear_moment(x);el=next((e for e in reversed(elements) if float(e["x0"])-1e-10<=x<=float(e["x1"])+1e-10),None);d,rot=_beam_response(el,x) if el else (0.0,0.0);samples.append({"x":x,"shear_kN":V,"moment_kNm":M,"deflection_mm":d,"rotation_rad":rot})

    jumps=[]
    for load in loads:
        if _kind(load.get("type")) in {"point","point_load"}:
            p=float(load["position"]);vl,ml=shear_moment(max(0.0,p-1e-8));vr,mr=shear_moment(p);jumps += [{"x":p,"shear_kN":vl,"moment_kNm":ml,"side":"left"},{"x":p,"shear_kN":vr,"moment_kNm":mr,"side":"right"}]

...
(()=>{
  ...
  function engineeringPoints(d){
    if(!d?.length||!window.beamEngineering)return {dangerous:[],cf:[]};
    const sfd=d.map(s=>({x:Number(s.x),y:Number(s.shear_kN)})).filter(p=>Number.isFinite(p.x)&&Number.isFinite(p.y));
    const bmd=d.map(s=>({x:Number(s.x),y:Number(s.moment_kNm)})).filter(p=>Number.isFinite(p.x)&&Number.isFinite(p.y));
    const jumps=lastResult?.diagrams?.jumps||[];
    const dangerous=window.beamEngineering.dangerousSections(sfd,jumps);
    const momentXs=(state.loads||[]).filter(l=>String(l.type).toLowerCase()==='moment').map(l=>Number(l.position));
    const excluded=[0,totalLength(),...(state.supports||[]).map(s=>Number(s.position)),...momentXs];
    const cf=window.beamEngineering.zeroCrossings(bmd,excluded);
    return {dangerous,cf,bmd};
  }
  ...
  function overview(){
    ...
    const forceUnit=typeof unitLabel==='function'?unitLabel('force'):'kN',momentUnit=typeof unitLabel==='function'?unitLabel('moment'):'kN·m';
    const f=x=>`${Math.abs(Number(x)).toFixed(2)} ${forceUnit}`;
    const m=x=>`${Math.abs(Number(x)).toFixed(2)} ${momentUnit}`;
    ...
    const ep=engineeringPoints(d);
    const critical=[];
    ep.dangerous.forEach(pt=>{
      const mv=window.beamEngineering.interpolate(ep.bmd,pt.x);
      critical.push(`<div class="criticalItem"><strong>Dangerous section:</strong> x = ${p(pt.x)}; BMD = ${m(mv??0)}</div>`);
    });
    ep.cf.forEach(pt=>critical.push(`<div class="criticalItem"><strong>Contraflexure:</strong> x = ${p(pt.x)}; BMD = 0 ${momentUnit}</div>`));
    const criticalHtml=critical.length?`<div class="criticalList">${critical.join('')}</div>`:'<p class="criticalNone">No dangerous section or contraflexure point was identified for the current loading and support arrangement.</p>';

    return {name,use,L,supports:supports.length,loads:loadPhrase,supportStory,loadStory:loadStories.join(' '),resultStory,fact,lenUnit,criticalHtml};
  }

  function renderOverview(){
    const host=$('#aiOverview');if(!host)return;
    const o=overview();
    host.innerHTML=`<div class="aiOverviewHead"><span class="aiBadge">AI structural briefing</span><h2>Your beam, decoded</h2></div><p><strong>Do you know this beam is called a ${o.name}?</strong> ${o.use.charAt(0).toUpperCase()+o.use.slice(1)}. This model is ${o.L.toFixed(2)} ${o.lenUnit} long with ${o.supports} support${o.supports===1?'':'s'} and ${o.loads}. ${o.supportStory}</p><div class="aiInsight"><strong>What is interesting about this particular beam?</strong> ${o.loadStory}</div><div class="aiSections"><div class="aiCritical"><h4>Critical sections</h4>${o.criticalHtml}</div><div class="aiSection"><h4>What the solution says</h4><p>${o.resultStory}</p></div><div class="aiSection"><h4>Engineering fact</h4><p>${o.fact}</p></div></div><div class="aiFacts"><div class="aiFact"><strong>Beam type</strong><span>${o.name}</span></div><div class="aiFact"><strong>Span</strong><span>${o.L.toFixed(2)} ${o.lenUnit}</span></div><div class="aiFact"><strong>Supports</strong><span>${o.supports}</span></div><div class="aiFact"><strong>Loading</strong><span>${o.loads}</span></div></div>`;
  }
